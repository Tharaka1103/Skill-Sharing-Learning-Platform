...
import com.example.skillshare.dto.JwtResponse;
...
import com.example.skillshare.repository.RoleRepository;
import com.example.skillshare.repository.UserRepository;
import com.example.skillshare.security.JwtUtils;
...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        try {
            ...
            logger.error("Authentication failed for user: {}, error: {}", loginRequest.getUsernameOrEmail(),
                    e.getMessage());
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null || strRoles.isEmpty()) {
            ...
            roles.add(userRole);
            logger.info("Setting default ROLE_USER for user: {}", signUpRequest.getUsername());
        } else {
                ...
                switch (role) {
                    case "admin":
                        Role adminRole = roleRepository.findByName(ERole.ROLE_ADMIN)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(adminRole);
                        logger.info("Adding ROLE_ADMIN for user: {}", signUpRequest.getUsername());
                        break;
                    case "mod":
                        Role modRole = roleRepository.findByName(ERole.ROLE_MODERATOR)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(modRole);
                        logger.info("Adding ROLE_MODERATOR for user: {}", signUpRequest.getUsername());
                        break;
                    default:
                        Role userRole = roleRepository.findByName(ERole.ROLE_USER)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(userRole);
                        logger.info("Adding ROLE_USER for user: {}", signUpRequest.getUsername());
                }
            });
        }
